...
<!-- The topmark symbol is generated from the calling template by calling it like this 
	...
	<xsl:param name="drawingPriority"/> <!-- drawingPriorityfor drawing instructions -->
		...
		<xsl:variable name="symbol">
			<xsl:choose>
				<xsl:when test="topmarkDaymarkShape = '1'">두표 (우현, 좌항로)</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '2'">TOPMAR04</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '3'">두표 (안전수역)</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '4'">두표 (고립장애)</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '5'">TOPMAR13</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '6'">TOPMAR14</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '7'">두표 (특수표지)</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '8'">TOPMAR17</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '9'">TOPMAR16</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '10'">두표 (서방위)</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '11'">두표 (동방위)</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '12'">TOPMAR14</xsl:when>
				<xsl:when test="topmarkDaymarkShape = '13'">두표 (북방위)</xsl:when>
        <xsl:when test="topmarkDaymarkShape = '14'">두표 (남방위)</xsl:when>
        ...
        <xsl:when test="topmarkDaymarkShape = '17'">TMARDEF2</xsl:when>
				...
				<xsl:when test="topmarkDaymarkShape = '19'">두표 (좌현, 우항로)</xsl:when>
