	...
	for isc, sectorCharacteristic in ipairs(feature.sectorCharacteristics) do
		for ils, lightSector in ipairs(sectorCharacteristic.lightSector) do
			...
			elseif lightSector.directionalCharacter then
				...
				featurePortrayal:AddInstructions('ClearGeometry;TextInstruction:' .. string.format('%03.0f deg', lightSector.directionalCharacter.orientation.orientationValue:ToNumber()) .. ',23,8')
				...
				featurePortrayal:AddInstructions('TextInstruction:' .. EncodeString(description) .. ',23,8')
			else
