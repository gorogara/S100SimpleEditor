...
bool	CClass_S101_Base::ParsingBaseCell(const BYTE *pData)
{
	...
	{
		AfxMessageBox(_T("File does not appear to have a valid ISO 8211 header!!"));
		return false;
	}
	else
	{
...
bool CClass_S101_Base::CheckSPAS()
{
	...
	{
		int cntSPAS = i->vecSPAS.size();
		if (cntSPAS > 0)
		{
			int cntRRNM = i->vecSPAS.begin()->vecRRNM.size();
	...
	for (auto i = m_stDSGI.vecFTCS.begin(); i != m_stDSGI.vecFTCS.end(); i++)
	{
		for (auto j = m_stDSGI.vecFTCS.begin(); j != m_stDSGI.vecFTCS.end(); j++)
		{
			if (i != j)
			{
				...
				{
					CString strErr;
					strErr.Format(_T("[%d] Feature Numeric code가 중복됩니다.\n"), i->nNumeric);
				...
				{
					CString strErr;
					strErr.Format(_T("[%s] Feautre Code가 중복됩니다.\n"), i->strCode);
					OutputDebugString(strErr);
					bCheck = false;
				}
			}
