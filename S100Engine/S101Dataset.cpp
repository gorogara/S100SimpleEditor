			...
			if (j->nRRNM == 120)
			{
				...
				if (pCurveRecord)
				{
					...
					if (subPoints.size() > 0)
					{
						...
						{
							for (int k = subPoints.size() - 1; k >= 0; k--)
							{
								points.push_back(subPoints.at(k));
							}
						}
...
void S101Dataset::CreateS100DrawingUnit(ID2D1Factory1* pDirect2dFactory, S100PCManager& s100PCManager)
{
	...
	for (auto i = displayList->m_vecInstruction.begin(); i != displayList->m_vecInstruction.end(); i++)
	{
		...
		if (geomType == 0)
		{
			OutputDebugString(_T("Drawing Instruction에서 가리킨 Feature의 공간타입이 없습니다.\n"));
			continue;
		}
		...
		if ((*i)->m_nTypeOfDrawingInstruction == areaInstruction)
		{
			if (geomType != 130)
			{
				OutputDebugString(_T("Area Instruction Type과 Feature Type의 공간타입 불일치!\n"));
				continue;
			}
			...
			if (pAreaInstruction->patternName.IsEmpty())
			{
				...
				pS100DrawingUnitPolygon->color = s100PCManager.s100ColorProfile.GetColor(L"Day", std::wstring(pAreaInstruction->colorName));
		...
		else if ((*i)->m_nTypeOfDrawingInstruction == lineInstruction)
		{
			...
			{
				CString strErr;
				strErr.Format(_T("Line Instruction Type과 Feature Type의 공간타입 불일치! (Feature ID : %d, Feature Type : %d)\n"), id, geomType);
			...
			pS100DrawingUnitPolyLine->color = s100PCManager.s100ColorProfile.GetColor(L"Day", std::wstring(pLineInstruction->colorName));
		...
		else if ((*i)->m_nTypeOfDrawingInstruction == pointInstruction)
		{
			...
			{
				CString strErr;
				strErr.Format(_T("Point Instruction Type과 Feature Type(%d)의 공간타입 불일치!\n"), geomType);
				OutputDebugString(strErr);
				continue;
			}
		...
		else if ((*i)->m_nTypeOfDrawingInstruction == textInstruction)
		{
			...
			if (pTextInstruction->pText)
			{
				for (auto j = pTextInstruction->pText->element.begin(); j != pTextInstruction->pText->element.end(); j++)
				{
					...
					{
						CString strErr;
						strErr.Format(_T("TextPoint Instruction Type과 Feature Type(%d)의 공간타입 불일치!\n"), geomType);
						OutputDebugString(strErr);
						continue;
					}
		...
		else if ((*i)->m_nTypeOfDrawingInstruction == augmentedPath)
		{
			...
			if (pAugmentedPath->pLineStyle)
			{
				if (typeid(*pAugmentedPath->pLineStyle) == typeid(LineStylesPackage::LineStyle))
				{
					...
					if (pLineStyle->pen)
					{
						pS100DrawingUnit->_color = s100PCManager.s100ColorProfile.GetColor(L"Day", pLineStyle->pen->color.token);
