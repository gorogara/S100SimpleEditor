	...
	ID2D1StrokeStyle1 *stroke)
{
	...
	if (fmodf(length, bitmapSize.width) == 0.0)
	{
		...
		pRenderTarget->DrawEllipse(D2D1::Ellipse(point1, 1, 1), pBrush);
		SafeRelease(&pBrush);
...
{
	if (InterSectLine(point1.x, point1.y, point2.x, point2.y, rect.left, rect.top, rect.right, rect.top)) 
	{
		return true;
	}
	else if (InterSectLine(point1.x, point1.y, point2.x, point2.y, rect.right, rect.top, rect.right, rect.bottom)) 
	{
		return true;
	}
	else if (InterSectLine(point1.x, point1.y, point2.x, point2.y, rect.left, rect.bottom, rect.right, rect.bottom)) 
	{
		return true;
	}
	else if (InterSectLine(point1.x, point1.y, point2.x, point2.y, rect.left, rect.top, rect.left, rect.bottom)) 
	{
		return true;
	}
						...
						{ 
							...
							{ 
								...
								DrawBitmapOnPolyline(
									...
									pDrawingUnit->screenPoints.size(), 
									pBitmap, 
									D2D1::RectF((FLOAT)rectView.left, (FLOAT)rectView.top, (FLOAT)rectView.right, (FLOAT)rectView.bottom));
								pRenderTarget->SetTransform(D2D1::Matrix3x2F::Identity());
								SafeRelease(&pBitmap);
