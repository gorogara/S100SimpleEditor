...
void S100PCManager::DrawLineStyle(std::wstring _name, ID2D1RenderTarget* pRenderTarget, ID2D1SolidColorBrush* pBrush, ID2D1StrokeStyle1* pStrokeStyle, std::wstring paletteName)
{
	...
	if (i != lineStyles.mapLineStyle.end())
	{
		...
		if (cntLineStyles > 1)
		{
			for (int j = 0; j < cntLineStyles; j++)
			{
				...
				if (pLineStyle->pen)
				{
					...
					color = s100ColorProfile.GetColor(paletteName, pLineStyle->pen->color.token);
		...
		else if (cntLineStyles == 1)
		{
			...
			if (pLineStyle)
			{
				pBrush->SetColor(s100ColorProfile.GetColor(paletteName, pLineStyle->pen->color.token));
...
void S100PCManager::DrawAreaFill(std::wstring _symbolName, ID2D1RenderTarget* pRenderTarget, ID2D1SolidColorBrush* pBrush, ID2D1StrokeStyle1* pStrokeStyle, std::wstring paletteName)
{
	...
	SVGReader* pSVG = s100SymbolManager.GetSVG(_symbolName + L"P");
	...
	if (pAreaFill->v1_y < 0.0001 && pAreaFill->v2_x < 0.0001)
	{
		...
		Draw(pAreaFill->_name + L"P", pRenderTarget, pBrush, pStrokeStyle, point1, 0, ENCCommon::DISPLAY_SYMBOL_SCALE, paletteName);
		Draw(pAreaFill->_name + L"P", pRenderTarget, pBrush, pStrokeStyle, point2, 0, ENCCommon::DISPLAY_SYMBOL_SCALE, paletteName);
		Draw(pAreaFill->_name + L"P", pRenderTarget, pBrush, pStrokeStyle, point3, 0, ENCCommon::DISPLAY_SYMBOL_SCALE, paletteName);
		Draw(pAreaFill->_name + L"P", pRenderTarget, pBrush, pStrokeStyle, point4, 0, ENCCommon::DISPLAY_SYMBOL_SCALE, paletteName);
	}
	else
	{
		D2D1_POINT_2F point1 = { 
		...
		Draw(pAreaFill->_name + L"P", pRenderTarget, pBrush, pStrokeStyle, point3, 0, ENCCommon::DISPLAY_SYMBOL_SCALE, paletteName);
		Draw(pAreaFill->_name + L"P", pRenderTarget, pBrush, pStrokeStyle, point4, 0, ENCCommon::DISPLAY_SYMBOL_SCALE, paletteName);
		Draw(pAreaFill->_name + L"P", pRenderTarget, pBrush, pStrokeStyle, point5, 0, ENCCommon::DISPLAY_SYMBOL_SCALE, paletteName);
	}
}
	...
	std::wstring paletteName)
{
	...
	for (auto i = pSVG->geometry.begin(); i != pSVG->geometry.end(); i++)
	{
		if (i->pGeometry)
		{
			if (i->bFill)
			{
				pBrush->SetColor(s100ColorProfile.GetColor(paletteName, i->fillColorName));
			...
			if (i->bStoke)
			{
				pBrush->SetColor(s100ColorProfile.GetColor(paletteName, i->strokeColorName));
				pBrush->SetOpacity(i->alpha);
...
void S100PCManager::CreateBitmapImage(ID2D1Factory1* pDirect2dFactory, IWICImagingFactory* pImagingFactory, ID2D1StrokeStyle1* pStrokeStyleS101Solid, std::wstring paletteName)
{
	for (auto i = areaFills.begin(); i != areaFills.end(); i++)
	{
		...
		if (pAreaFill)
		{
			...
			CString str;
			str.Format(_T("%s %0.1lf %0.1lf %0.1lf %0.1lf\n"), i->_name.c_str(), rect.left, rect.top, rect.right, rect.bottom);
...
D2D1_RECT_F S100PCManager::GetAreaFillRect(AreaFill* pAreaFill, FLOAT scale)
{
	...
	SVGReader* pSVG = s100SymbolManager.GetSVG(pAreaFill->_name + L"P");
