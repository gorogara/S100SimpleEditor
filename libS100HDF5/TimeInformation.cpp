...
char* TimeInformation::DateTimeOfFirstRecordAttributeName = (char*)"dateTimeOfFirstRecord";
char* TimeInformation::DateTimeOfLastRecordAttributeName = (char*)"dateTimeOfLastRecord";
char* TimeInformation::TimeRecordIntervalAttributeName = (char*)"timeRecordInterval";
	...
	int TimeRecordInterval)
{
	...
	else if (period > 0)
	{
		...
		{
			_NumberOfTimes = (int)((period / _TimeRecordInterval) + 1);
		}
		else
		{
			_NumberOfTimes = 0;
...
int TimeInformation::GetNearestTimeIndex(time_t time)
{
	...
	findIndex = (int)(offset / _TimeRecordInterval);
